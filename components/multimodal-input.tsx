...
import { TopicInput } from '@/components/toolUI/topicInput';
...
function PureMultimodalInput({
  ...
  const { topicInputValues, isTopicInputComplete, setIsTopicInputComplete, sourceIds, setSourceIds } = useChatContext();
  useEffect(() => {
  ...
  useEffect(() => {
    // when the component mounts, the input field is populated with The DOM value if available, Otherwise, the stored value from localStorage.
    if (textareaRef.current) {
  ...
  // updates localStorage whenever the input state (input) changes.
  useEffect(() => {
    ...
    <div className="relative w-full flex flex-col gap-4">
      ...
      {isTopicInputComplete && (
        <>
          ...
          <Textarea
            ref={textareaRef}
            placeholder={`Send a message on ${chatTopics.find((chatTopic) => chatTopic.id === topicInputValues.topicId)?.name}`}
            value={input}
